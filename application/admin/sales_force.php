...
class Sales_force extends CI_Model{
    ...
    public function get_value_formato($id,$name,$html = ''){
    ...
    public function get_multiple($id,$name){

        $get = $this->db->query("SELECT * FROM sf_formato_campos  WHERE id_formato = ".$id." AND name like '%".$name."%' ");
    ...
    public function get_alertas($id,$status,$date){

        if($id == 1):
            $v = ' AND sf_alertas.id_vendedor <>1 ';
        else:
            $v = ' AND sf_alertas.id_vendedor = '.$id.' ';
        endif;

        $q = 'SELECT * FROM sf_alertas left join sf_tipo_accion on id_tipo_accion = tipo_alerta left join sf_cuentas on sf_alertas.id_cuenta = sf_cuentas.id_cuenta  where fecha_alerta = "'.$date.'" '.$v.' AND status_alerta = '.$status.' order by fecha_alerta DESC';
...
// 'SELECT * FROM sf_alertas left join sf_tipo_alerta on id_tipo_alerta = tipo_alerta where id_vendedor = '.$id.' AND status_alerta = '.$status.' LIMIT 10'
    ...
    public function get_servicios_ultima_accion($id_folio){
        ...
        $get = $this->db->get('sf_tipo_servicio');

        foreach ($get->result() as $v) :
        $servicios [$v->id_tipo_servicio] = $v->nombre_servicio;    
        endforeach;
    ...
    public function get_services_format($folio_accion){
        ...
        if($get->num_rows() > 0):
            ...
            if($formatos->num_rows() > 0):
                ...
                foreach ($formatos->result() as $v):
                    $result []= $v->nombre_servicio;                                             
                endforeach;
    ...
    public function get_vendedores_cta($id_cuenta){

        $get = $this->db->query('SELECT sf_vendedores.* FROM sf_cuentas_vendedores 
                    left join sf_vendedores on sf_cuentas_vendedores.id_vendedor = sf_vendedores.id_vendedor
                    WHERE sf_cuentas_vendedores.id_cuenta = '.$id_cuenta.' AND sf_vendedores.id_vendedor <> 1');
        ...
        if($get->num_rows() > 0):
            ...
            foreach ($get->result() as $row) :
                $vendedores [$row->id_vendedor] = $row->nombre_vendedor.' '.$row->ap_vendedor ; 
            endforeach;
    ...
    public function validate_tipo_accion($folio_accion){

        $get = $this->db->get_where('sf_accion', array('folio_accion'=>$folio_accion));
        ...
        if($get->num_rows() > 0):
            ...
            foreach ($get->result() as $row) :
                ...
                if($row->tipo_accion == 11):

                    array_push($no_disponibles, 8);

                elseif ($row->tipo_accion == 5) :

                    array_push($no_disponibles, 5);
                
                elseif ($row->tipo_accion == 4) :

                    array_push($no_disponibles, 5, 1,3,2,11,8);
        ...
        $resultado = array_unique($no_disponibles);

        foreach ($resultado as $key => $value) :
            $query .=' AND id_tipo_accion <> '.$value;
        endforeach;

        $q = 'select  * from  sf_tipo_accion WHERE accion <> "" '.$query.' ORDER BY sf_tipo_accion.order ASC ';
    ...
    public function get_template($tipo_servicio){
